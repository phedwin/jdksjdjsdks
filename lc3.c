...
void update_flags(uint16_t r) {
	if (reg[r] == 0) {
		reg[R_COND] = FL_ZRO;
		   ...
		   reg[r] < 0) /* a 1 in the left-most bit indicates negative */
	{
		reg[R_COND] = FL_NEG;
	} else {
		reg[R_COND] = FL_POS;
...
int main(int argc, const char *argv[]) {
	...
	/* since exactly one condition flag should be set at any given time, set
	 * the Z flag */
	reg[R_COND] = FL_ZRO;
	...
	while (running) {
		...
		switch (op) {
			...
			case OP_STI: {
				...
				uint16_t pc_offset =
				    sign_extend(instr & 0x1FF, 9);
				mem_write(mem_read(reg[R_PC] + pc_offset),
					  reg[r0]);
			} break;
			case OP_STR: {
				uint16_t r0 = (instr >> 9) & 0x7;
			...
			case OP_TRAP:
				...
				switch (instr & 0xFF) {
					...
					case TRAP_PUTSP: {
						/* one char per byte (two bytes
						   per word) here we need to
